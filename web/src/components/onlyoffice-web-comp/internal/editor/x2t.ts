...
export class X2tConverter {
  /**
   * @description 单个 worker 实例连续处理的转换次数上限。
   * x2t wasm 模块在单次初始化后会持续占用并可能泄漏内存，
   * 达到上限后主动销毁并重建 worker，释放 wasm 堆栈与文件系统状态。
   */
  private static readonly RESET_AFTER_CONVERSIONS = 20;
   ...
   * 同时立即终止 worker，避免损坏的 wasm 实例继续服务后续转换。
   */
   ...
   * 若当前 worker 发生错误或被判定为需要回收，会自动重建并重试一次。
   */
  public async convert(
    ...
    try {
      return await this.doConvert(params, logger);
    } catch (error) {
      this.logRaw("error", "convert failed, recycling worker", [
        "[X2tConverter] Convert failed, will recycle worker and retry once:",
        error,
      ]);
      this.terminate(logger);
      return this.doConvert(params, logger);
    }
  }

  /**
   * @description 执行单次转换；超过连续转换上限后会先销毁 worker。
   */
  private async doConvert(
    ...
    if (this.conversionCount >= X2tConverter.RESET_AFTER_CONVERSIONS) {
      this.logRaw("log", "worker conversion limit reached, recycling", [
        `[X2tConverter] Worker reached ${X2tConverter.RESET_AFTER_CONVERSIONS} conversions, recycling to free wasm memory`,
      ]);
      this.terminate(logger);
    }
    ...
    const result = await this.sendMessage<X2tConvertResult>("convert", payload);
