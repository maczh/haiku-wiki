...
- `cd web && npm run build` → `rsync -a --delete web/dist/ server/internal/static/dist/`，**之后补回 `.gitkeep`**（否则 build-embed.sh 静默跳过 go build）。
...
- **H5 手机版**：`web/src/h5/**`；`useViewMode()` 必为 zustand 全局 store（用 useState 切桌面/手机不生效）；`/share`、`/doc-share` 须在 App 层放行给桌面路由。
