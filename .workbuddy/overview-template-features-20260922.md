...
## 需求#1 一致性收口（追加）
排查所有「选模板」入口，发现**文库工作台「常用模板」**（`BookDashboard` 的 `TemplateQuickPanel`）经 `openNewDocFromTemplate` 直开「选择位置」step1、**绕过预览**，违反需求#1。
...
- 其余入口已合规：全局首页常用模板经 `QuickStartModal`（渲染 `template.content` 预览）；模板中心经画廊预览；+菜单经画廊预览。
- 验证：`verify-tpl-dash.sh` 6/6（点卡先弹预览、不直建、用后入选择位置）；回归 run-all 6 套 166/166 全绿。

## 最终验证总账
| 套件 | 结果 |
|------|------|
| 自研 API 验证 `verify-tpl-api.sh` | 16/16 |
| 自研 UI 验证 `verify-tpl-ui.sh`（预览后使用/模板中心/另存为模板/管理员改删/导入不白屏） | 23/23 |
| 自研 UI 验证 `verify-tpl-dash.sh`（文库工作台常用模板先预览） | 6/6 |
| 回归 run-all（6 套） | 166/166 |
